  ...
  public:
    explicit NavigateActionServer(
        ...
        _odomSubscription = create_subscription<nav_msgs::msg::Odometry>(
            ...
            std::bind(
                &NavigateActionServer::odometryCallback, this,
                std::placeholders::_1
            ),
            odom_options
        );
        ...
        _xPid.set_gains(
            ...
            -std::numeric_limits<double>::infinity(), antiWindupStrategy
        );
        _yPid.set_gains(
            0.1, 0.0, 0.0, std::numeric_limits<double>::infinity(),
            -std::numeric_limits<double>::infinity(), antiWindupStrategy
  ...
  private:
    ...
    struct Trajectory3 {
        ...
        explicit Trajectory3(
            ...
            const Eigen::Vector3d& endVelocity, double attack, double decay,
            double totalTime, const Eigen::Vector3d& targetPosition
        ) :
          x(Trajectory(
              startVelocity.x(), endVelocity.x(), attack, decay, totalTime,
              targetPosition.x()
          )),
          y(Trajectory(
              startVelocity.y(), endVelocity.y(), attack, decay, totalTime,
              targetPosition.y()
          )),
          z(Trajectory(
              startVelocity.z(), endVelocity.z(), attack, decay, totalTime,
              targetPosition.z()
          )) { }
    ...
    struct OrientationSolver {
        ...
        explicit OrientationSolver(
          ...
          finalOrientation(finalOrientation), totalTime(totalTime) {
            if (initialOrientation.dot(finalOrientation) < 0.0) {
                Eigen::Quaterniond second(
                    -finalOrientation.w(), -finalOrientation.x(),
        ...
        Eigen::Vector3d error(
            double                    elapsed,
            const Eigen::Quaterniond& currentOrientation /*world frame*/
        ) const {
    ...
    geometry_msgs::msg::Twist computeCommandVelocity(
        ...
        const OrientationSolver& orientationSolver, double elapsed,
        ...
        const auto relativePosition =
            currentPosition
            - initialPosition;    // world, relative to the initial position, as
                                  // if guppy's initial position were (0, 0, 0)
        const auto error =
            relativeTargetPosition
            - relativePosition;    // end - start; target - current; gives
        ...
        auto localTargetVelocity =
            currentOrientation.inverse() * targetVelocity;
        ...
        commandVelocity.linear.x =
            ...
            + _xPid.compute_command(
                localError.x(), rclcpp::Duration::from_seconds(delta)
            );
        commandVelocity.linear.y =
            localTargetVelocity.y()
            + _yPid.compute_command(
                localError.y(), rclcpp::Duration::from_seconds(delta)
            );
        commandVelocity.linear.z =
            localTargetVelocity.z()
            + _zPid.compute_command(
                localError.z(), rclcpp::Duration::from_seconds(delta)
            );
    ...
    void execute(
        ...
        const Eigen::Vector3d initialPosition(
            initialState.pose.position.x, initialState.pose.position.y,
        ...
        const Eigen::Vector3d initialVelocity(
            ...
            initialState.twist.linear.z
        );    // world frame
        const auto relativeFinalPosition =
            finalPosition
            - initialPosition;    // vector between guppy's initial position
        ...
        while (rclcpp::ok()) {
            ...
            auto commandVelocity = computeCommandVelocity(
                trajectory, orientationSolver, elapsed, delta, state /*world*/,
