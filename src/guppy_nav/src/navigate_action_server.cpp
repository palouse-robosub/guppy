...
class NavigateActionServer : public rclcpp::Node {
 public:
  explicit NavigateActionServer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions()) : Node("navigate_action_server", options) {
    ...
    auto handleGoal = [this](const rclcpp_action::GoalUUID& uuid, std::shared_ptr<const guppy_msgs::action::Navigate::Goal> goal) {
      RCLCPP_INFO(this->get_logger(), "goal request with pose and relative set to %d",
    ...
    _actionServer = rclcpp_action::create_server<guppy_msgs::action::Navigate>(this, "/navigate", handleGoal, handleCancel, handleAccepted,
 ...
 private:
  ...
  struct Trajectory3 {
    ...
    Eigen::Vector3d velocity(double elapsed) const { return Eigen::Vector3d(x.getTargetVelocity(elapsed), y.getTargetVelocity(elapsed), z.getTargetVelocity(elapsed)); }

    Eigen::Vector3d position(double elapsed) const { return Eigen::Vector3d(x.getTargetPosition(elapsed), y.getTargetPosition(elapsed), z.getTargetPosition(elapsed)); }
  };
  ...
  geometry_msgs::msg::Twist computeCommandVelocity(const Trajectory3& trajectory,
    ...
    const Eigen::Vector3d currentPosition(state.pose.position.x, state.pose.position.y,
                                          state.pose.position.z);     // world
    const auto relativePosition = currentPosition - initialPosition;  // world, relative to the initial position, as if
                                                                      // guppy's initial position were (0, 0, 0)
    const auto error = relativeTargetPosition - relativePosition;     // end - start; target - current; gives vector from
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<guppy_msgs::action::Navigate>> goalHandle) {
    ...
    const Eigen::Vector3d goalPosition(goal->pose.position.x, goal->pose.position.y,
                                       goal->pose.position.z);  // if local == true, relative to guppy; if local == false it
    ...
    if (goal->local) {
      finalPosition = initialPosition + initialOrientation.inverse() * goalPosition;  // get final position in world based on
    ...
    const Eigen::Vector3d initialVelocity(initialState.twist.linear.x, initialState.twist.linear.y,
                                          initialState.twist.linear.z);  // world frame
    const auto relativeFinalPosition = finalPosition - initialPosition;  // vector between guppy's initial position (world) and
                                                                         // target final position (world), position as if guppy
                                                                         // were (0, 0, 0)

    Trajectory3 trajectory(initialVelocity, Eigen::Vector3d(0.0, 0.0, 0.0), ATTACK, DECAY, goal->timeout,
                           relativeFinalPosition);  // world frame velocities (will output target
                                                    // velocities in world frame)
    OrientationSolver orientationSolver(initialOrientation, finalOrientation,
