...
private:
    ...
    struct Trajectory3 {
        ...
        explicit Trajectory3(
            ...
            const Eigen::Vector3d& end_velocity, double attack, double decay,
            double total_time, const Eigen::Vector3d& target_position
        ) :
        x(Trajectory(
            start_velocity.x(), end_velocity.x(), attack, decay, total_time,
            target_position.x()
        )),
        y(Trajectory(
            start_velocity.y(), end_velocity.y(), attack, decay, total_time,
            target_position.y()
        )),
        z(Trajectory(
            start_velocity.z(), end_velocity.z(), attack, decay, total_time,
            target_position.z()
        )) { }
...
struct OrientationSolver {
    ...
    explicit OrientationSolver(
        ...
        Eigen::Quaterniond&       final_orientation /*world frame*/,
        double                    total_time
    ) :
      initial_orientation(initial_orientation),
      final_orientation(final_orientation), total_time(total_time) {
        if (initial_orientation.dot(final_orientation) < 0.0) {
            Eigen::Quaterniond second(
                -final_orientation.w(), -final_orientation.x(),
                -final_orientation.y(), -final_orientation.z()
            );
    ...
    Eigen::Vector3d error(
        double                    elapsed,
        const Eigen::Quaterniond& current_orientation /*world frame*/
    ) const {
...
private:
    ...
    geometry_msgs::msg::Twist computeCommandVelocity(
        ...
        const OrientationSolver& orientation_solver, double elapsed,
        double delta, const KinematicState& state,
        const Eigen::Vector3d& initial_position
    ) {
        const auto relative_target_position = trajectory.position(
            elapsed
        );    // relative to guppy starting position, world frame
        const auto target_velocity =
            trajectory.velocity(elapsed);    // world frame
        ...
        const auto relative_position =
            current_position
            - initial_position;    // world, relative to the initial position, as
                                   // if guppy's initial position were (0, 0, 0)
        const auto error =
            relative_target_position
            - relative_position;    // end - start; target - current; gives
        ...
        auto local_target_velocity =
            current_orientation.inverse() * target_velocity;
        const auto local_error = current_orientation.inverse() * error;
        const auto angular_error_local =
            orientation_solver.error(elapsed, current_orientation);

        geometry_msgs::msg::Twist command_velocity;

        command_velocity.angular.z = yaw_pid_.compute_command(
            angular_error_local.z(), rclcpp::Duration::from_seconds(delta)
        );
        ...
        command_velocity.linear.x =
            ...
            + x_pid_.compute_command(
                local_error.x(), rclcpp::Duration::from_seconds(delta)
            );
        command_velocity.linear.y =
            local_target_velocity.y()
            + y_pid_.compute_command(
                local_error.y(), rclcpp::Duration::from_seconds(delta)
            );
        command_velocity.linear.z =
            local_target_velocity.z()
            + z_pid_.compute_command(
                local_error.z(), rclcpp::Duration::from_seconds(delta)
            );
    ...
    void execute(
        ...
        const auto relative_final_position =
            final_position
            - initial_position;   // vector between guppy's initial position
                                  // (world) and target final position (world),
                                  // position as if guppy were (0, 0, 0)

        Trajectory3 trajectory(
            initial_velocity, Eigen::Vector3d(0.0, 0.0, 0.0), NavigateActionServer::attack, NavigateActionServer::decay,
            goal->timeout,
        ...
        while (rclcpp::ok()) {
            ...
            current_position << state.pose.position.x, state.pose.position.y,
                state.pose.position.z;
