...
private:
    ...
    bool               cancel_ = false;
public:
    explicit PoseSetterServer(const rclcpp::NodeOptions& options = rclcpp::NodeOptions()) :
        ...
        auto handle_goal = [this](
            ...
            RCLCPP_INFO(
                ...
                rclcpp_action::to_string(id).c_str(), goal->pose.position.x, goal->pose.position.y,
                goal->pose.position.z, goal->pose.orientation.w, goal->pose.orientation.x,
        ...
        setter_   = this->create_client<guppy_msgs::srv::SetHoldPose>("reset_holding_pose");
        odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
            "/odometry/filtered", quality::volatile_profile,
            [this](std::shared_ptr<nav_msgs::msg::Odometry> msg) { this->odometry_callback(*msg); }
        );
        state_sub_ = this->create_subscription<guppy_msgs::msg::State>(
            "/state", quality::keep_last_profile,
            [this](const std::shared_ptr<const guppy_msgs::msg::State> msg) { state_callback(*msg); }
        );
    }
private:
    ...
    void execute(
        ...
        Eigen::Vector3d end_pos(goal->pose.position.x, goal->pose.position.y, goal->pose.position.z);
        ...
        while (rclcpp::ok()) {
            if (cancel_ || goal_handle->is_canceling() || state_ != guppy_msgs::msg::State::NAV) {
            ...
            auto qdistance = current_quat_.inverse() * quat_list[setpoint_index];
