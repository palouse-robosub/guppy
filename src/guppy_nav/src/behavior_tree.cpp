...
private:
    ...
    const std::shared_ptr<rclcpp::Node> change_state_client_ =
        std::make_shared<rclcpp::Node>("chage_state_behavior_client");
    const std::shared_ptr<rclcpp::Node> navigation_client_ =
        std::make_shared<rclcpp::Node>("navigate_behavior_client");
    const std::shared_ptr<rclcpp::Node> detection_subscriber_ =
...
public:
    NavigationBehaviorTree() : Node("navigation_behavior_tree") {
        ...
        BT::RosNodeParams detection_parameters(this->detection_subscriber_, "/cam/test/detections");

        this->factory_->registerNodeType<ChangeStateBehavior>("ChangeState", state_parameters);
        this->factory_->registerNodeType<NavigateBehavior>("Navigate", navigate_parameters);
        this->factory_->registerNodeType<FaceDetectionBehavior>(
            "FaceDetection", navigate_parameters
        );
        this->factory_->registerNodeType<AcquireDetection>("AcquireDetection", detection_parameters);
        ...
        timer_ = this->create_wall_timer(
            std::chrono::milliseconds(NavigationBehaviorTree::tick_ms), tick
        );
...
private:
    void initialize_tree() {
        ...
        RCLCPP_INFO(
            this->get_logger(), "Behavior tree %s initialized with %lu registered nodes.",
