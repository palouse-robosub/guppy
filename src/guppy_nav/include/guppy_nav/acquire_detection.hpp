...
class AcquireDetection : public BT::RosTopicSubNode<guppy_msgs::msg::CornerDetectionList> {
 public:
  AcquireDetection(const std::string& name, const BT::NodeConfig& config, const BT::RosNodeParams& params) : BT::RosTopicSubNode<guppy_msgs::msg::CornerDetectionList>(name, config, params) {}

  static BT::PortsList providedPorts() { return providedBasicPorts({BT::InputPort<std::string>("target"), BT::OutputPort<guppy_msgs::msg::CornerDetection>("detection")}); }
 ...
 private:
  static bool matchesTarget(const guppy_msgs::msg::CornerDetection detection, const std::string& target) { return detection.name == target; }
};
