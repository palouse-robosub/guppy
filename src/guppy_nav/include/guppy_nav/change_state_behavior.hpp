...
class ChangeStateBehavior : public BT::RosServiceNode<guppy_msgs::srv::ChangeState> {
 public:
  ChangeStateBehavior(const std::string& name, const BT::NodeConfig& conf, const BT::RosNodeParams& params) : RosServiceNode<guppy_msgs::srv::ChangeState>(name, conf, params) {
    RCLCPP_INFO(logger(), "ChangeState behavior initialized.");
  }

  static BT::PortsList providedPorts() { return providedBasicPorts({BT::InputPort<uint8_t>("state")}); }
