 ...
 public:
  CombineSensors() : Node("combine_sensors") {
    ...
    reset_service = this->create_service<std_srvs::srv::Empty>(
        "reset_odom",
        [&](const std::shared_ptr<std_srvs::srv::Empty::Request> request, std::shared_ptr<std_srvs::srv::Empty::Response> response) {
          initial_pose.x = odom.pose.pose.position.x;
          initial_pose.y = odom.pose.pose.position.y;
          initial_pose.z = odom.pose.pose.position.z;
          initial_orientation = Eigen::Quaterniond(odom.pose.pose.orientation.w, odom.pose.pose.orientation.x, odom.pose.pose.orientation.y, odom.pose.pose.orientation.z);
        },
        10);
  ...
  void dvl_callback(nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    Eigen::Quaterniond q_world_body(odom.pose.pose.orientation.w, odom.pose.pose.orientation.x, odom.pose.pose.orientation.y, odom.pose.pose.orientation.z);
    ...
    Eigen::Vector3d omega(odom.twist.twist.angular.x, odom.twist.twist.angular.y, odom.twist.twist.angular.z);
