  ...
  public:
    FlirPublisher() : Node("camera_publisher") {
        ...
        for (unsigned int i = 0; i < camList.GetSize(); ++i) {
            ...
            for (CamInfo& c : cams) {
                if (this->camList[i]->GetDeviceSerialNumber().c_str()
                    == c.serial) {
                    pos = c.position;
                    break;
                }
            ...
            this->pubs_.push_back(
                image_transport::create_publisher(this, topic)
            );
            // this->publishers_.push_back(this->create_publisher<sensor_msgs::msg::Image>(topic,
            // 1));
            this->threads_.emplace_back(
                &FlirPublisher::acquireFootage, this, i
            );
        }
    }
    ...
    void acquireFootage(int camIndex) {
        ...
        while (this->running_) {
            ...
            sensor_msgs::fillImage(
                *msg, sensor_msgs::image_encodings::BAYER_RGGB8,
                pImg->GetHeight(), pImg->GetWidth(), pImg->GetStride(),
                pImg->GetData()
            );
            pImg->Release();
