...
class PerspectiveNPoint : public rclcpp::Node {
 public:
  PerspectiveNPoint() : Node("pnp") {
    corner_sub_ = this->create_subscription<guppy_msgs::msg::CornerDetectionList>("/cam/test/detections", 10, std::bind(&PerspectiveNPoint::callback, this, std::placeholders::_1));
    info_sub_ = this->create_subscription<sensor_msgs::msg::CameraInfo>("/cam/test/camera_info", 10, std::bind(&PerspectiveNPoint::info_callback, this, std::placeholders::_1));

    output_pub_ = this->create_publisher<guppy_msgs::msg::TransformList>("/cam/test/transforms", 10);
  }
  ...
  void callback(guppy_msgs::msg::CornerDetectionList::UniquePtr msg) {
    ...
    for (int result_i = 0; result_i < msg->detections.size(); result_i++) {
      ...
      RCLCPP_DEBUG(this->get_logger(), "%f\t%f\t%f\t%f\t%f\t%f\t\t%s", tvec.at<double>(0, 0), tvec.at<double>(1, 0), tvec.at<double>(2, 0), rvec.at<double>(0, 0), rvec.at<double>(1, 0),
                   rvec.at<double>(2, 0), detection.name.c_str());
  ...
  void info_callback(sensor_msgs::msg::CameraInfo::UniquePtr msg) { this->camera_info_ = *msg; }
