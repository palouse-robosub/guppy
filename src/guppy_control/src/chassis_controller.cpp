...
void ChassisController::update_current_state(const nav_msgs::msg::Odometry& msg) {
    ...
    const Eigen::Vector<double, 6> new_current_vel(
        ros_twist.linear.x, ros_twist.linear.y, ros_twist.linear.z, ros_twist.angular.x,
        ros_twist.angular.y, ros_twist.angular.z
    );
...
void ChassisController::update_desired_state(const geometry_msgs::msg::Twist& msg) {
    ...
    if (request->type == request->guppy_msgs::srv::SetHoldPose::Request::RESET) {
        ...
        if (request->type == request->guppy_msgs::srv::SetHoldPose::Request::GLOBAL) {
            ...
            this->desired_position_state_ =
                this->current_position_state_ + (this->current_orientation_state_.inverse() * pos);
        }
    }
}
...
void ChassisController::update_parameters(const Parameters& parameters) {
    ...
    qp_H +=
        Eigen::Matrix<double, T200Interface::motor_count, T200Interface::motor_count>::Identity()
        * 0.1;

    // no equality constraints
    const Eigen::MatrixXd qp_C =
        Eigen::MatrixXd::Identity(T200Interface::motor_count, T200Interface::motor_count);
    ...
    this->qp_.init(
        qp_H, std::nullopt, std::nullopt, std::nullopt, qp_C, this->parameters_.motor_lower_bounds,
        this->parameters_.motor_upper_bounds
    );
    ...
    for (size_t i = 0; i < 3; ++i)
        this->velocity_pid[i].set_gains(
            ...
            this->parameters_.pid_gains_vel_linear[2], std::numeric_limits<double>::infinity(),
            -std::numeric_limits<double>::infinity(), ChassisController::anti_windup_strategy
        );
    ...
    for (size_t i = 3; i < 6; ++i)
        this->velocity_pid[i].set_gains(
            ...
            this->parameters_.pid_gains_vel_angular[2], std::numeric_limits<double>::infinity(),
            -std::numeric_limits<double>::infinity(), ChassisController::anti_windup_strategy
        );
    ...
    for (size_t i = 0; i < 3; ++i)
        this->pose_pid[i].set_gains(
            ...
            this->parameters_.pid_gains_pose_linear[2], std::numeric_limits<double>::infinity(),
            -std::numeric_limits<double>::infinity(), ChassisController::anti_windup_strategy
        );

    for (size_t i = 3; i < 6; ++i)
        this->pose_pid[i].set_gains(
            this->parameters_.pid_gains_pose_angular[0],
            this->parameters_.pid_gains_pose_angular[1],
            this->parameters_.pid_gains_pose_angular[2], std::numeric_limits<double>::infinity(),
            -std::numeric_limits<double>::infinity(), ChassisController::anti_windup_strategy
        );
...
Eigen::Vector<double, T200Interface::motor_count>
    ChassisController::allocate_thrust(const Eigen::Vector<double, 6>& local_wrench) {
    // turn the least squares problem solution set into a QP program
    const Eigen::VectorXd qp_g =
        -(this->parameters_.motor_coefficients.transpose() * this->parameters_.axis_weight_matrix
          * local_wrench);

    // update solver and solve
    this->qp_.update(
        std::nullopt, qp_g, std::nullopt, std::nullopt, std::nullopt, std::nullopt, std::nullopt
    );
    this->qp_.solve();
...
Eigen::Vector3d ChassisController::calculate_rotational_nudge(bool debug) {
    ...
    if (debug) {
        ...
        std::cout << "old_state: " << static_cast<unsigned int>(this->current_orientation_lock_)
                  << '\n';
        std::cout << "c: " << this->current_orientation_state_.w() << " "
                  << this->current_orientation_state_.vec().transpose() << '\n';
        std::cout << "d: " << this->desired_orientation_state_.w() << " "
                  << this->desired_orientation_state_.vec().transpose() << '\n';
...
bool ChassisController::control_loop(const bool debug) {
    ...
    const Eigen::Vector<double, 6> desired_squared =
        this->desired_velocity_state_.cwiseAbs().array() * this->desired_velocity_state_.array();
    ...
    Eigen::Vector3d gravity_force(
        0, 0, -(ChassisController::gravity * this->parameters_.robot_mass)
    );
    gravity_force = this->current_orientation_state_.inverse() * gravity_force;
    const Eigen::Vector<double, 6> gravity_wrench(
        gravity_force[0], gravity_force[1], gravity_force[2], 0, 0, 0
    );

    // calculate buoyant effect on sub
    const Eigen::Vector3d buoyancy_force(
        0, 0,
        this->parameters_.water_density * this->parameters_.robot_volume * ChassisController::gravity
    );
    const Eigen::Vector3d r_vec =
        this->current_orientation_state_.inverse() * this->parameters_.center_of_buoyancy;
    Eigen::Vector3d buoyancy_torque = 1 * r_vec.cross(buoyancy_force);
    // if (pose_pid_enabled) {
    buoyancy_torque = -1 * r_vec.cross(buoyancy_force);
    // }
    const Eigen::Vector3d buoyancy_force_rotated =
        this->current_orientation_state_.inverse() * buoyancy_force;
    ...
    for (int i = 0; i < 3; i++) {
        if (abs(desired_velocity_state_[i]) < this->parameters_.pose_lock_deadband[i]
            && this->pose_pid_enabled_)
    ...
    for (size_t i = 0; i < T200Interface::motor_count; ++i) {
        double max_in_dir  = this->motor_forces_[i] < 0 ? this->parameters_.motor_lower_bounds[i]
...
void ChassisController::loop_runner(bool debug) {
    ...
    while (this->is_thread_running_.load()) {
        ...
        this->parameter_mutex_.lock();
        const bool okay = control_loop(
            debug
        );    // maybe pass in parameters here instead of locking during entire control loop runtime?
        this->parameter_mutex_.unlock();

        // time the loop
        const auto stop = std::chrono::high_resolution_clock::now();
        const long duration_us =
