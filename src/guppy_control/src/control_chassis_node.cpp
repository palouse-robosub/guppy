...
class ControlChassis : public rclcpp::Node {
  public:
    ControlChassis() : Node("control_chassis") {
        ...
        this->param_subscriber_ =
            std::make_shared<rclcpp::ParameterEventHandler>(
                this
            );    // subscribe to parameter change event

        // parameter callback to update parameters on event

        auto param_callback = [this](
            ...
            auto param_map = get_param_map(
                &controller_params
            );    // map pointers of parameter to parameter names
            ...
            for (const auto& param : parameter_event.changed_parameters) {
                ...
                // get entries (from maps below) with values of lambda/pointer
                // based on the parameter name key
                auto pointer_it = param_map.find(
                    name
                );    // get pointer to parameter member in struct
                      // (pointer_it->second refers to value)
                auto transformer_it = transformers.find(
                    name
                );    // get transformer lambda to change simple
                      // vector to complex eigen type
                auto printer_it = printers.find(
                    name
                );    // get lambda for formatting complex type into string

                std::visit(
                    [&](
                        auto* pointer,
                        auto& transformer
                    ) {    // call lambda with variant types
                        ...
                        using R = std::remove_cv_t<
                            std::remove_reference_t<std::invoke_result_t<
                                ...
                                const rclcpp::Parameter&>>>;    // final result
                                                                // of calling
                                                                // lambda
                                                                // transformer,
                                                                // ie.
                        ...
                        if constexpr (std::is_same_v<
                                          P, R>) {    // ensure transformed type
                                                      // and type at pointer are
                                                      // the same (so you don't
                                                      // set bad data)
                            std::string before, after;
                            before = printer_it->second(
                                static_cast<const void*>(pointer)
                            );    // ignore type of pointer as you pass to
                                  // printer to format for debugging

                            auto tmp = transformer(
                                value
                            );    // store transformed value (from double vector
                                  // to actual parameter type ie. Eigen::Matrix)
                                  // based on parameter name

                            after = printer_it->second(
                                static_cast<const void*>(&tmp)
                            );    // ignore type and format result for debugging

                            *pointer =
                                std::move(tmp);    // fast copy for vector

                            update = true;    // parameter struct is dirty and
                                              // should be updated
                    ...
                    pointer_it->second,
                    transformer_it->second
                );    // pass values into visit, must belong to
                      // defined variants
            }
        ...
        controller =
            new ChassisController(parameters, thruster_interface, 100000);

        // setup motor publishers for sim
        for (int i = 0; i < 8; i++) {
            sim_motor_publishers_[i] =
        ...
        odom_subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
            ...
            std::bind(
                &ControlChassis::odom_callback, this, std::placeholders::_1
            )
        );
        cmd_vel_subscription_ =
            this->create_subscription<geometry_msgs::msg::Twist>(
                "/cmd_vel", 10,
                std::bind(
                    &ControlChassis::cmdvel_callback, this,
                    std::placeholders::_1
                )
            );
        state_subscription_ = this->create_subscription<guppy_msgs::msg::State>(
            "/state", 10,
            std::bind(
                &ControlChassis::state_callback, this, std::placeholders::_1
            )
        );
  ...
  private:
    ...
    std::unordered_map<std::string, ParameterTypes> transformers = {
        { "motor_positions", std::function<Eigen::Matrix<double, 6, N_MOTORS>(
        ...
        { "motor_lower_bounds",
         std::function<
              Eigen::Matrix<double, N_MOTORS, 1>(const rclcpp::Parameter&)>(
              [](const rclcpp::Parameter& value) {
    ...
    void load_params_(ChassisController::ChassisControllerParams* params) {
        ...
        for (auto& [name, pointer_to] : param_map) {
            if (!this->has_parameter(name)) {
                RCLCPP_WARN(
                    this->get_logger(), "Parameter '%s' not set, skipping.",
                    name.c_str()
                );
                continue;
            }
            ...
            std::visit(
                [&](auto* pointer, auto& transformer) {
                    ...
                    using R = std::invoke_result_t<
                        decltype(transformer), const rclcpp::Parameter&>;
    ...
    void print_params_struct_(
        ...
        auto param_map = get_param_map(
            params
        );    // get pointers to parameter struct members by parameter name

        for (auto& [name, pointer_to] : param_map) {
            const auto printer_it = printers.find(name);

            if (printer_it
                == printers.end()) {    // skip if no formatting lambda
                RCLCPP_WARN(
                    this->get_logger(),
            ...
            std::visit(
                [&](auto* pointer) {
                    rendered =
                        printer_it->second(static_cast<const void*>(pointer));
                },
                pointer_to    // pass in pointer
            );

            RCLCPP_INFO(
                this->get_logger(), "%s: %s", name.c_str(), rendered.c_str()
            );
        }
    }
    ...
    static std::string eigen_to_str(
        const Eigen::MatrixBase<Derived>& matrix, const Eigen::IOFormat& format
    ) {
        std::ostringstream oss;
    ...
    static Eigen::Matrix<double, R, C>
        ...
        if (vec.size() != R * C) {
            ...
            throw std::runtime_error(
                "Bad matrxc parameter passed to controller."
            );
        }
    ...
    template <int N>
    static Eigen::Matrix<double, 6, N>
        to_motor_coefficients(const std::vector<double>& flat5N) {
    ...
    // parameters
    std::unordered_map<std::string, ParameterPointerTypes>
        get_param_map(ChassisController::ChassisControllerParams* params) {
        return {
