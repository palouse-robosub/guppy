...
class ControlChassis : public rclcpp::Node {
 public:
  ControlChassis() : Node("control_chassis") {
    ...
    this->declare_parameter<std::vector<double>>(
        "motor_positions",
        std::vector<double>(5 * N_MOTORS, 0.0));  // flattened 6xN
    this->declare_parameter<std::vector<double>>(
    ...
    this->declare_parameter<std::vector<double>>(
        "axis_weight_matrix",
        std::vector<double>(6 * 6, 0.0));  // flattened 6 * 6
    this->declare_parameter<std::vector<double>>(
    ...
    this->declare_parameter<std::vector<double>>("pose_lock_deadband",
                                                 std::vector<double>(6, 0.0));
    this->declare_parameter<std::vector<double>>("drag_coefficients",
                                                 std::vector<double>(6, 0.0));
    this->declare_parameter<std::vector<double>>("drag_areas",
                                                 std::vector<double>(6, 0.0));
    this->declare_parameter<std::vector<double>>(
        "drag_effect_matrix",
        std::vector<double>(6 * 6, 0.0));  // flattened 6x6
    ...
    this->param_subscriber_ = std::make_shared<rclcpp::ParameterEventHandler>(
        this);  // subscribe to parameter change event

    // parameter callback to update parameters on event

    auto param_callback = [this](const rcl_interfaces::msg::ParameterEvent&
                                     parameter_event) {
      ...
      auto controller_params =
          this->controller
              ->get_param_struct();  // get copy of current parameters

      auto param_map = get_param_map(
          &controller_params);  // map pointers of parameter to parameter names

      bool update = false;

      for (const auto& param : parameter_event.changed_parameters) {
        const auto name =
            rclcpp::Parameter::from_parameter_msg(param).get_name();
        ...
        auto pointer_it =
            param_map.find(name);  // get pointer to parameter member in struct
                                   // (pointer_it->second refers to value)
        auto transformer_it =
            transformers.find(name);  // get transformer lambda to change simple
                                      // vector to complex eigen type
        auto printer_it = printers.find(
            name);  // get lambda for formatting complex type into string
        ...
        std::visit(
            [&](auto* pointer,
              ...
              if constexpr (std::is_same_v<P, R>) {  // ensure transformed type
                ...
                before = printer_it->second(static_cast<const void*>(
                    pointer));  // ignore type of pointer as you pass to printer
                                // to format for debugging

                auto tmp = transformer(
                    value);  // store transformed value (from double vector to
                             // actual parameter type ie. Eigen::Matrix) based
                             // on parameter name

                after = printer_it->second(static_cast<const void*>(
                    &tmp));  // ignore type and format result for debugging

                *pointer = std::move(tmp);  // fast copy for vector

                update =
                    true;  // parameter struct is dirty and should be updated

                RCLCPP_INFO(this->get_logger(),
                            "Parameter '%s' changed (%s)->(%s)", name.c_str(),
    ...
    this->param_event_callback_handle_ =
        param_subscriber_->add_parameter_event_callback(param_callback);

    thruster_interface = new T200Interface(
        "can0", {0x411, 0x412, 0x413, 0x414, 0x415, 0x416, 0x417, 0x418});

    controller = new ChassisController(parameters, thruster_interface, 100000);

    // setup motor publishers for sim
    for (int i = 0; i < 8; i++) {
      sim_motor_publishers_[i] = this->create_publisher<std_msgs::msg::Float32>(
          "/sim/motor_forces/m_" + std::to_string(i), 10);
    }

    // setup subscriptions
    odom_subscription_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "/odometry/filtered", 10,
        std::bind(&ControlChassis::odom_callback, this, std::placeholders::_1));
    cmd_vel_subscription_ =
        this->create_subscription<geometry_msgs::msg::Twist>(
            "/cmd_vel", 10,
            std::bind(&ControlChassis::cmdvel_callback, this,
                      std::placeholders::_1));
    state_subscription_ = this->create_subscription<guppy_msgs::msg::State>(
        "/state", 10,
        std::bind(&ControlChassis::state_callback, this,
                  std::placeholders::_1));

    reset_service = this->create_service<guppy_msgs::srv::SetHoldPose>(
        "reset_holding_pose",
        [&](const std::shared_ptr<guppy_msgs::srv::SetHoldPose::Request>
                request,
 ...
 private:
  ...
  using ParameterPointerTypes =
      std::variant<  // expected complex types for parameters
  ...
  using ParameterTypes =
      std::variant<  // expected complex lambdas for transformers
          std::function<Eigen::Matrix<double, 6, N_MOTORS>(
              const rclcpp::Parameter&)>,
          std::function<Eigen::Matrix<double, N_MOTORS, 1>(
              const rclcpp::Parameter&)>,
  ...
  std::unordered_map<std::string, ParameterTypes> transformers = {
      {"motor_positions",
       std::function<Eigen::Matrix<double, 6, N_MOTORS>(
           const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) {
         return ControlChassis::to_motor_coefficients<N_MOTORS>(
             value.as_double_array());
       })},
      {"motor_lower_bounds",
       std::function<Eigen::Matrix<double, N_MOTORS, 1>(
           const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) {
      ...
      {"water_density",
       std::function<double(const rclcpp::Parameter&)>(
           [](const rclcpp::Parameter& value) { return value.as_double(); })},
      {"robot_volume",
       std::function<double(const rclcpp::Parameter&)>(
           [](const rclcpp::Parameter& value) { return value.as_double(); })},
      {"robot_mass",
  ...
  std::unordered_map<std::string, std::function<std::string(const void*)>>
      printers = {
          {"motor_positions",
           [](const void* pointer) {
             auto& matrix =
                 *static_cast<const Eigen::Matrix<double, 6, N_MOTORS>*>(
                     pointer);
          ...
          {"axis_weight_matrix",
           [](const void* pointer) {
             auto& matrix =
                 *static_cast<const Eigen::Matrix<double, 6, 6>*>(pointer);
             return eigen_to_str(matrix, LineFormat);
           }},
          {"drag_effect_matrix",
           [](const void* pointer) {
          ...
          {"motor_lower_bounds",
           [](const void* pointer) {
             auto& vec =
                 *static_cast<const Eigen::Matrix<double, N_MOTORS, 1>*>(
                     pointer);
          ...
          {"pose_lock_deadband",
           [](const void* pointer) {
             auto& vec =
                 *static_cast<const Eigen::Matrix<double, 6, 1>*>(pointer);
             return eigen_to_str(vec, InlineFormat);
           }},
          {"drag_coefficients",
           [](const void* pointer) {
          ...
          {"center_of_buoyancy",
           [](const void* pointer) {
             auto& vec =
                 *static_cast<const Eigen::Matrix<double, 3, 1>*>(pointer);
             return eigen_to_str(vec, InlineFormat);
           }},
          {"pid_gains_vel_linear",
           [](const void* pointer) {
             return vec_to_str(
                 *static_cast<const std::vector<double>*>(pointer));
           }},
          {"pid_gains_vel_angular",
           [](const void* pointer) {
             return vec_to_str(
  ...
  void load_params_(ChassisController::ChassisControllerParams* params) {
    ...
    for (auto& [name, pointer_to] : param_map) {
      if (!this->has_parameter(name)) {
        RCLCPP_WARN(this->get_logger(), "Parameter '%s' not set, skipping.",
                    name.c_str());
        continue;
      }
      ...
      std::visit(
          [&](auto* pointer, auto& transformer) {
            using P =
                std::remove_cv_t<std::remove_reference_t<decltype(*pointer)>>;
            using R = std::invoke_result_t<decltype(transformer),
                                           const rclcpp::Parameter&>;
  ...
  void print_params_struct_(
      ChassisController::ChassisControllerParams* params) {
    auto param_map = get_param_map(
        params);  // get pointers to parameter struct members by parameter name
    ...
    for (auto& [name, pointer_to] : param_map) {
      ...
      if (printer_it == printers.end()) {  // skip if no formatting lambda
        RCLCPP_WARN(this->get_logger(),
                    "No printer registered for '%s', skipping.", name.c_str());
        continue;
      }
  ...
  static std::string eigen_to_str(const Eigen::MatrixBase<Derived>& matrix,
  ...
  template <int N>
  static Eigen::Matrix<double, N, 1> to_eigen_vec(
  ...
  static Eigen::Matrix<double, R, C> to_eigen_matrix(
      const std::vector<double>& vec) {
    if (vec.size() != R * C) {
  ...
  template <int N>
  static Eigen::Matrix<double, 6, N> to_motor_coefficients(
      const std::vector<double>& flat5N) {
  ...
  // parameters
  std::unordered_map<std::string, ParameterPointerTypes> get_param_map(
      ChassisController::ChassisControllerParams* params) {
