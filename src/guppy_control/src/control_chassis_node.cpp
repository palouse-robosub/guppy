...
class ControlChassis : public rclcpp::Node {
 public:
  ControlChassis() : Node("control_chassis") {
    ...
    auto param_callback = [this](const rcl_interfaces::msg::ParameterEvent& parameter_event) {
      ...
      for (const auto& param : parameter_event.changed_parameters) {
        ...
        // get entries (from maps below) with values of lambda/pointer based on
        // the parameter name key
        auto pointer_it = param_map.find(name);         // get pointer to parameter member in struct
                                                        // (pointer_it->second refers to value)
        auto transformer_it = transformers.find(name);  // get transformer lambda to change simple
                                                        // vector to complex eigen type
        auto printer_it = printers.find(name);          // get lambda for formatting complex type into string

        std::visit(
            [&](auto* pointer,
                auto& transformer) {  // call lambda with variant types
                                      // (map<string, ParameterTypes> and
                                      // PaarameterPointerTypes)
              if (transformer_it == transformers.end()) {
                RCLCPP_INFO(this->get_logger(), "No transformer found for parameter '%s', skipping.", name.c_str());
                return;
              }

              using P = std::remove_cv_t<std::remove_reference_t<decltype(*pointer)>>;  // get type at pointer/reference and
                                                                                        // strip const/volatile
              using R = std::remove_cv_t<std::remove_reference_t<std::invoke_result_t<decltype(transformer),
                                                                                      const rclcpp::Parameter&>>>;  // final result of calling
                                                                                                                    // lambda transformer, ie.
                                                                                                                    // transformed type

              if constexpr (std::is_same_v<P, R>) {  // ensure transformed type
                                                     // and type at pointer are
                                                     // the same (so you don't
                                                     // set bad data)
                std::string before, after;
                before = printer_it->second(static_cast<const void*>(pointer));  // ignore type of pointer as you pass to printer
                                                                                 // to format for debugging

                auto tmp = transformer(value);  // store transformed value (from double vector to
                                                // actual parameter type ie. Eigen::Matrix) based
                                                // on parameter name
            ...
            pointer_it->second,
            transformer_it->second);  // pass values into visit, must belong to
                                      // defined variants
      }
    ...
    reset_service = this->create_service<guppy_msgs::srv::SetHoldPose>(
        "reset_holding_pose",
        [&](const std::shared_ptr<guppy_msgs::srv::SetHoldPose::Request> request, std::shared_ptr<guppy_msgs::srv::SetHoldPose::Response> response) { controller->reset_holding_pose(request); }, 10);
    ...
    timer_ = this->create_wall_timer(10ms,
                                     timer_callback);  // publishes sim motor thrust every 10 milliseconds
  ...
  void odom_callback(nav_msgs::msg::Odometry::SharedPtr msg) { controller->update_current_state(msg); }

  void cmdvel_callback(geometry_msgs::msg::Twist::SharedPtr msg) { controller->update_desired_state(msg); }
 ...
 private:
  ...
  // transform simple vector/double types into complex eigen types with a
  // lambda, based on parameter name
  std::unordered_map<std::string, ParameterTypes> transformers = {
      {"motor_positions", std::function<Eigen::Matrix<double, 6, N_MOTORS>(const rclcpp::Parameter&)>(
                              [](const rclcpp::Parameter& value) { return ControlChassis::to_motor_coefficients<N_MOTORS>(value.as_double_array()); })},
      {"motor_lower_bounds",
       std::function<Eigen::Matrix<double, N_MOTORS, 1>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return to_eigen_vec<N_MOTORS>(value.as_double_array()); })},
      {"motor_upper_bounds",
       std::function<Eigen::Matrix<double, N_MOTORS, 1>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return to_eigen_vec<N_MOTORS>(value.as_double_array()); })},
      {"axis_weight_matrix", std::function<Eigen::Matrix<double, 6, 6>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return to_eigen_matrix<6, 6>(value.as_double_array()); })},
      {"pid_gains_vel_linear", std::function<std::vector<double>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return value.as_double_array(); })},
      {"pid_gains_vel_angular", std::function<std::vector<double>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return value.as_double_array(); })},
      {"pid_gains_pose_linear", std::function<std::vector<double>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return value.as_double_array(); })},
      {"pid_gains_pose_angular", std::function<std::vector<double>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return value.as_double_array(); })},
      {"pose_lock_deadband", std::function<Eigen::Matrix<double, 6, 1>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return to_eigen_vec<6>(value.as_double_array()); })},
      {"drag_coefficients", std::function<Eigen::Matrix<double, 6, 1>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return to_eigen_vec<6>(value.as_double_array()); })},
      {"drag_areas", std::function<Eigen::Matrix<double, 6, 1>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return to_eigen_vec<6>(value.as_double_array()); })},
      {"drag_effect_matrix", std::function<Eigen::Matrix<double, 6, 6>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return to_eigen_matrix<6, 6>(value.as_double_array()); })},
      {"water_density", std::function<double(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return value.as_double(); })},
      {"robot_volume", std::function<double(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return value.as_double(); })},
      {"robot_mass", std::function<double(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return value.as_double(); })},
      {"center_of_buoyancy", std::function<Eigen::Matrix<double, 3, 1>(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return to_eigen_vec<3>(value.as_double_array()); })},
      {"qp_epsilon", std::function<double(const rclcpp::Parameter&)>([](const rclcpp::Parameter& value) { return value.as_double(); })}};
  ...
  std::unordered_map<std::string, std::function<std::string(const void*)>> printers = {
      ...
      {"pid_gains_vel_linear", [](const void* pointer) { return vec_to_str(*static_cast<const std::vector<double>*>(pointer)); }},
      {"pid_gains_vel_angular", [](const void* pointer) { return vec_to_str(*static_cast<const std::vector<double>*>(pointer)); }},
      {"pid_gains_pose_linear", [](const void* pointer) { return vec_to_str(*static_cast<const std::vector<double>*>(pointer)); }},
      {"pid_gains_pose_angular", [](const void* pointer) { return vec_to_str(*static_cast<const std::vector<double>*>(pointer)); }},
      {"water_density", [](const void* pointer) { return std::to_string(*static_cast<const double*>(pointer)); }},
      {"robot_volume", [](const void* pointer) { return std::to_string(*static_cast<const double*>(pointer)); }},
      {"robot_mass", [](const void* pointer) { return std::to_string(*static_cast<const double*>(pointer)); }},
      {"qp_epsilon", [](const void* pointer) { return std::to_string(*static_cast<const double*>(pointer)); }}};

  T200Interface* thruster_interface;
  ChassisController* controller;

  // set given parameter struct to current ros2 parameters using
  ...
  void print_params_struct_(ChassisController::ChassisControllerParams* params) {
    ...
    for (auto& [name, pointer_to] : param_map) {
      ...
      std::string rendered;
      std::visit([&](auto* pointer) { rendered = printer_it->second(static_cast<const void*>(pointer)); },
  ...
  static Eigen::Matrix<double, N, 1> to_eigen_vec(const std::vector<double>& vec) {
    if (vec.size() != N) {
      // RCLCPP_ERROR(this->get_logger(), "Bad vector parameter passed to
      // controller!");
  ...
  static Eigen::Matrix<double, R, C> to_eigen_matrix(const std::vector<double>& vec) {
    if (vec.size() != R * C) {
      // RCLCPP_ERROR(this->get_logger(), "Bad matrix parameter passed to
      // controller!");
  ...
  // maps pointers to members of a parameter struct to the names of the
  // parameters
  std::unordered_map<std::string, ParameterPointerTypes> get_param_map(ChassisController::ChassisControllerParams* params) {
    return {{"motor_positions", &params->motor_coefficients},
  ...
  */
  static Eigen::Matrix<double, 6, 1> get_single_motor_coefficients(double x, double y, double z, double phi, double theta) {
    ...
    out << sinp * cost, sinp * sint, cosp, (z * sinp * sint) - (y * cosp), (x * cosp) - (z * sinp * cost), (y * sinp * cost) - (x * sinp * sint);
    return out;
  }
};
