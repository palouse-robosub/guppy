...
public:
    CanRx() : Node("can_rx"), socket_("can0", this->get_logger()), parameter_sub_(std::make_shared<rclcpp::ParameterEventHandler>(this)) {
...
private:
    ...
    publisher_shared_ptr create_id_publisher(canid_t id) {
        ...
        const auto publisher = this->create_publisher<guppy_msgs::msg::CanFrame>(topic_str, quality::volatile_profile);
