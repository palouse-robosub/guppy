...
class CanRx : public rclcpp::Node {
  ...
  private:
    ...
    bool setup_can_socket() {
        ...
        ifreq       ifr{};
        const char* can_net =
            this->get_parameter("interface").as_string().c_str();
    ...
    int create_id_publisher(const int id) {
        for (int i = 0; i < MAX_PUBLISHERS; i++) {
            if (ids_[i] == 0) {
                ...
                publishers_[i] =
                    this->create_publisher<guppy_msgs::msg::CanFrame>(
                        topic_str, 10
                    );
                RCLCPP_INFO(
                    this->get_logger(), "created publisher %s\r\n",
                    topic_str.c_str()
                );
                return i;
            }
    ...
    void can_loop() {
        while (running_.load()) {
            ...
            if (nbytes < 0) {
                RCLCPP_WARN(
                    this->get_logger(), "Reading from CAN frame failed"
                );
                continue;
            }
            ...
            if (int idx = check_id(frame.can_id); idx != -1) {
                ...
                if (idx != -1) {
                    ...
                    RCLCPP_ERROR(
                        this->get_logger(),
                        "Failed to create publisher for CAN ID %x", frame.can_id
                    );
                }
            }
        }
